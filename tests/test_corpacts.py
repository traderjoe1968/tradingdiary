...
CorpActFields = namedtuple(
    "CorpActFields",
    ["uniqueid", "units", "total", "reportdate", "ticker", "cusip", "secname"],
)
...
class CorpActPreprocessingMixin(object):
    ...
    def testPreprocessCorpActs(self):
        ...
        corp_acts = [
            tx for tx in self.statement.transactions
            if isinstance(tx, Types.CorporateAction)
        ]
        group = self.reader.preprocessCorporateActions(
            self.session,
            {
                (sec.uniqueidtype, sec.uniqueid): sec
                for sec in self.statement.securities
            },
            corp_acts,
        )
...
# Test case classes
class CorpActCancelTestCase(
    ReadXmlSnippetMixin, CorpActPreprocessingMixin, unittest.TestCase
):
    """
    ...
    preprocessed_txs = [
        ...
        (
            CorpActKey(
                dttrade=datetime(2012, 5, 15, 20, 25),
                type=ibflex.enums.Reorg.MERGER,
...
class SubscriptionSpanningMultipleDatesTestCase(
    ReadXmlSnippetMixin, CorpActPreprocessingMixin, unittest.TestCase
):
    """
    Tests the hack in flex.reader.FlexStatementReader.tender() and
    flex.reader.merge_reorg() to correct cost basis for
    ...
    preprocessed_txs = [
        (
            CorpActKey(
                dttrade=datetime(2015, 2, 19, 19, 45),
                type=ibflex.enums.Reorg.TENDER,
        ...
        (
            CorpActKey(
                dttrade=datetime(2015, 2, 25, 20, 25),
                type=ibflex.enums.Reorg.MERGER,
...
class BondMaturityTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
...
class DelistTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
...
class ChangeSecurityTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
...
class OversubscribeTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
...
class RightsIssueTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
...
class SplitWithCusipChangeTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
...
class StockDividendTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
    ...
    def persisted_txs(self):
        ...
        return [
            inventory.Split(
                ...
                fiaccount=self.account,
                security=self.securities[0],
                units=Decimal("1090.909091"),
                numerator=Decimal("12"),
                denominator=Decimal("11"),
            )
        ]


class SpinoffTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
    ...
    extra_securities = [
        {
            "name": "GYRODYNE CO OF AMERICA INC - 10.89 NON-TRANSFERABLE NOTES",
            "ticker": "GYRO.NOTE",
            "uniqueidtype": "CONID",
            "uniqueid": "144464070",
        }
...
class SubscribeRightsBadIsinToTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
    ...
    def persisted_txs(self):
        ...
        return [
            inventory.Exercise(
                ...
                units=Decimal("3839"),
                fromsecurity=self.securities[1],
                fromunits=Decimal("-3839"),
            )
        ]


class MergerBadIsinFromTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    """
...
class CashMergerTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
...
class KindMergerTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
...
class CashKindMergerTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    """
...
class TenderTestCase(ReadXmlSnippetMixin, unittest.TestCase):
    stmt_sections = [
        """
        <CorporateActions>
